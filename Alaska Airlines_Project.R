...
Loss_LTS <- lin.season.pred$mean - future.ts
Loss_LTS
...
names(Reg_LTS.df) <- c("Time Period","Actual Revenue","Forecasted Revenue","Loss of Revenue")
Reg_LTS.df

total_loss_LTS <- sum(Reg_LTS.df$`Loss of Revenue`, na.rm = TRUE) # TOTAL LOSS OF REVENUE (2020-2023)
total_revenue_LTS <- sum(Reg_LTS.df$`Actual Revenue`, na.rm = TRUE) # FORECASTED REVENUE (2020-2023)
percentage_loss_LTS <- (total_loss_LTS / total_revenue_LTS) * 100 # PERCENTAGE LOSS OF REVENUE (2020-2023)
cat("Total Loss of Revenue:", total_loss_LTS, "\n")
...
# ESTIMATION OF LOSS OF REVENUE
Loss_QTS <- quad.season.pred$mean - future.ts
Loss_QTS

# COMPARISON OF FORECAST FOR FUTURE PERIOD
Reg_QTS.df <- round(data.frame((time(future.ts)),future.ts, quad.season.pred$mean, Loss_QTS),3)
names(Reg_QTS.df) <- c("Time Period","Actual Revenue","Forecasted Revenue","Loss of Revenue")
Reg_QTS.df

total_loss_QTS <- sum(Reg_QTS.df$`Loss of Revenue`, na.rm = TRUE) # TOTAL LOSS OF REVENUE (2020-2023)
total_revenue_QTS <- sum(Reg_QTS.df$`Actual Revenue`, na.rm = TRUE) # FORECASTED REVENUE (2020-2023)
percentage_loss_QTS <- (total_loss_QTS / total_revenue_QTS) * 100 # PERCENTAGE LOSS OF REVENUE (2020-2023)
cat("Total Loss of Revenue:", total_loss_QTS, "\n")
